...
void AbbRobot::trackMove()
{
    ...
    inter_waypoints=quinticInterpolation(waypoints,0.001);
    ...
    {   
        const auto currentTime = Clock::now();
         const Duration desiredDuration(1.0 / 1000.0);
...
void AbbRobot::abbWrite(trajectory_msgs::JointTrajectoryPoint point)
{
    ...
    //clamp(&abbManipulator_.jointList[5].pos_des, 0.0, 0.0);//第五个关节
    for (int i = 0; i < joint_count; ++i)
    {
        abbManipulator_.jointList[i].pos_des = abbManipulator_.jointList[i].pos_des * directionMotor_[i]; 
        motorsInterface->control_pos_vel(*motorsInterface->getMotor(i+1),abbManipulator_.jointList[i].pos_des,10.0);
    }
